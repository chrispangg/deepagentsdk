...
<Callout title="Runtime Required" type="info">The CLI works with Node.js (>= 18) or Bun (>= 1.0.0)</Callout>
...
<Callout type="tip">Run without installing using `npx`, `pnpm dlx`, `bunx`, or `yarn dlx`.</Callout>
...
```bash
# Start a new session
npx deepagentsdk --session my-project
...
<Accordions type="single">
  <Accordion title="CLI won't start">
    ...
    - Ensure Node.js >= 18 or Bun >= 1.0.0 is installed
  </Accordion>
