...
<Callout title="Node.js Required" type="warn">The CLI requires Node.js 24.11.1</Callout>
...
<Tabs items={['Run without installing', 'Install globally', 'Install locally', 'From source']}>
  ...
  <Tab value="From source">
    ```bash
    git clone https://github.com/chrispangg/deepagentsdk.git
    cd deepagentsdk
    npm install
    npm run cli
    ```
...
```bash
# Start a new session
npx deep-agent --session my-project
