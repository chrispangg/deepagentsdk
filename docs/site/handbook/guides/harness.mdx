...
The **agent harness** is the core runtime that provides deep agents with their advanced capabilities. It wraps AI SDK's `ToolLoopAgent` with built-in tools and features that enable complex, multi-step reasoning.
...
In addition to any user-defined subagents, deep agents have access to a `general-purpose` subagent at all times:
