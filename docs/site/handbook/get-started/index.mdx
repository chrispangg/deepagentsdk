...
<Tabs items={['npm', 'pnpm', 'bun', 'yarn']}>
  ...
  <Tab value="yarn">
    ...
    # CLI usage
    yarn dlx deepagentsdk
    ```
  </Tab>
</Tabs>

<Callout type="info">The CLI can be run without installing using `npx`, `pnpm dlx`, `bunx`, or `yarn dlx`.</Callout>
...
```bash
ANTHROPIC_API_KEY=your-key npx tsx agent.ts
```
...
<Accordions type="single">
  <Accordion title="Cannot find module 'deepagentsdk'">
    ...
    **Solution:** Ensure the package is installed and your TypeScript config is correct:

    ```bash
    # Install the package
    npm install deepagentsdk
    # or: pnpm add deepagentsdk, bun add deepagentsdk, yarn add deepagentsdk
    ```

    Check your `tsconfig.json` has proper module resolution settings.
  </Accordion>
...
```bash
# Start CLI (works with any package manager)
npx deepagentsdk
