...
The agent uses AI SDK's `messages` array for conversation history. When streaming with events, always pass `event.messages` back to maintain context.
</Callout>
...
<Callout title="Important" type="warn">
The library requires AI SDK `LanguageModel` instances instead of string-based model IDs.
</Callout>
