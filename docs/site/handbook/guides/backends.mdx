---
title: Backends
description: Choose and configure filesystem backends for Deep Agent
---
...
<Mermaid chart={`
flowchart TD
    A["lsInfo('/persistent/src/')"] --> B{Check Routes}
    B --> C["/persistent/"]
    B --> D["/persistent/cache/"]
    B --> E["/shared/"]

    C --> F{Longest Match?}
    F -->|Yes| G["Use PersistentBackend"]
    G --> H["Strip prefix: '/src/'"]
    H --> I["Call backend.lsInfo('/src/')"]
    I --> J["Add prefix back"]
    J --> K["['/persistent/src/main.ts']"]
`} />
...
```typescript
import { CompositeBackend, StateBackend, FilesystemBackend, PersistentBackend } from 'deepagentsdk';
...
import { BackendProtocol, FileInfo, FileData } from 'deepagentsdk';
