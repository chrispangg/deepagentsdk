...
# Specify a model
bunx deepagentsdk --model anthropic/claude-sonnet-4-20250514

# Use a specific working directory
bunx deepagentsdk --dir ./my-project

# Enable session persistence
bunx deepagentsdk --session my-session
```
...
# Examples:
bunx deepagentsdk --cache
bunx deepagentsdk --model openai/gpt-4o --no-cache  # Not needed for OpenAI
```
...
```bash
# Start a session
bunx deepagentsdk --session refactor-auth
...
bunx deepagentsdk --model claude-haiku-4-5-20251001

# Complex refactoring (smart, capable)
bunx deepagentsdk --model claude-sonnet-4-20250514
```
...
bunx deepagentsdk --session feature-x-development
...
bunx deepagentsdk --session bug-fix-session
```
...
bunx deepagentsdk --dir ./frontend
...
bunx deepagentsdk --model openai/gpt-4o --no-cache

# Aggressive eviction for large file operations
bunx deepagentsdk --eviction-limit 10000
...
```bash
bunx deepagentsdk --session my-new-session
```
