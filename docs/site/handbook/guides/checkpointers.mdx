...
```typescript
import { FileSaver } from 'deepagentsdk';
...
```typescript
import { KeyValueStoreSaver, InMemoryStore } from 'deepagentsdk';
...
<Mermaid chart={`
flowchart TD
    A[Start Session] --> B{messages provided?}
    B -->|Yes, non-empty| C[Use provided messages]
    B -->|Yes, empty array| D[Clear checkpoint, start fresh]
    B -->|No| E{prompt provided?}
    E -->|Yes| F[Convert prompt to message]
    E -->|No| G{threadId exists?}
    G -->|Yes| H[Load checkpoint history]
    G -->|No| I[Start with empty history]

    C --> J[Continue with messages]
    D --> J
    F --> J
    H --> J
    I --> J
...
```typescript
import type { BaseCheckpointSaver, Checkpoint } from 'deepagentsdk';
...
$ npm run cli --session my-project
