...
# Specify a model
npx deep-agent --model anthropic/claude-sonnet-4-20250514

# Use a specific working directory
npx deep-agent --dir ./my-project

# Enable session persistence
npx deep-agent --session my-session
```

### Development

```bash
# Run from source (in the deepagentsdk repository)
npm run cli

# With options
npm run cli --model openai/gpt-4o --dir ./workspace
```
...
# Examples:
npx deep-agent --cache
npx deep-agent --model openai/gpt-4o --no-cache  # Not needed for OpenAI
```
...
```bash
# Start a session
npx deep-agent --session refactor-auth
...
npx deep-agent --model claude-haiku-4-5-20251001

# Complex refactoring (smart, capable)
npx deep-agent --model claude-sonnet-4-20250514
```
...
npx deep-agent --session feature-x-development
...
npx deep-agent --session bug-fix-session
```
...
npx deep-agent --model openai/gpt-4o --no-cache

# Aggressive eviction for large file operations
npx deep-agent --eviction-limit 10000
...
```bash
npx deep-agent --session my-new-session
```
...
For more help, see [GitHub Issues](https://github.com/chrispangg/deepagentsdk/issues).
